...
public class PutRespConverter implements RiakResponseConverter<List<RiakObject>> {

  private final ByteArrayWrapper key;
  private final ByteArrayWrapper bucket;

  public PutRespConverter(ByteArrayWrapper key, ByteArrayWrapper bucket) {
    this.key = key;
    this.bucket = bucket;
  }
  ...
  public List<RiakObject> convert(RiakMessage message) throws ExecutionException
  {
    ...
    else // We have a reply and it isn't not-found
    {
      try
      {
        RiakKvPB.RpbPutResp resp = RiakKvPB.RpbPutResp.parseFrom(data);
        ...
        if (count == 0)
        {
          RiakKvPB.RpbPutResp.Builder responseBuilder = resp.toBuilder();
        ...
        for (int i = 0; i < count; i++)
        {
          ...
          RiakObject riakObject =
            RiakObject.create(bucket.unsafeGetValue())
              .setKey(resp.hasKey() ? resp.getKey().toByteArray(): key.unsafeGetValue())
