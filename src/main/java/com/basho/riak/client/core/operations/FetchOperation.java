...
public class FetchOperation<T> extends FutureOperation<T>
{
	...
	{
		GetRespConverter getConverter = new GetRespConverter(bucket, key);
		List<RiakObject> riakObjects = getConverter.convert(rawResponse);
	...
	protected RiakMessage createChannelMessage()
	{
		...
		{
			builder.setTimeout(fetchMeta.getTimeout());
		}

		if (fetchMeta.hasNval())
		{
			builder.setNVal(fetchMeta.getNval());
		}

		if (fetchMeta.hasSloppyQuorum())
		{
			builder.setSloppyQuorum(fetchMeta.getSloppyQuorum());
		}
